...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        // TODO(proj2): implement
        //Chris: check if there is a Duplicate key
        if (keys.contains(key)) {
            throw new BPlusTreeException("Can't put a duplicate key");
        }
        ...
        //case 2
            ...
            LeafNode right_node_page = new LeafNode(metadata, bufferManager, rightKeys, rightRids, rightSibling, treeContext);
            long right_node_page_num = right_node_page.getPage().getPageNum();
            this.rightSibling = Optional.of(right_node_page_num);
            this.keys = leftKeys;
            this.rids = leftRids;

            sync();
            return Optional.of(new Pair<DataBox, Long>(rightKeys.get(0), this.rightSibling.get()));
        }
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        // TODO(proj2): implement
        int order = metadata.getOrder();
        int maxLeafSize = (int) Math.ceil(2 * order * fillFactor);
        //Chris: starting put one by one
        while(data.hasNext()){
            ...
            //If excede maxLeafSize we split it like put function
            if(keys.size() > maxLeafSize){
                List<DataBox> leftKeys = keys.subList(0, maxLeafSize);
                List<DataBox> rightKeys = keys.subList(maxLeafSize, keys.size());
                List<RecordId> leftRids = rids.subList(0, maxLeafSize);
                List<RecordId> rightRids = rids.subList(maxLeafSize, rids.size());
    ...
    public static LeafNode fromBytes(BPlusTreeMetadata metadata, BufferManager bufferManager,
        ...
        // Jay: similar to InnerNode::fromBytes, account for the difference how the LeafNode is serialized.
        ...
        // -1 if we don't have sibling
        if (rightSiblingNum == -1) {
            rightSibling = Optional.empty();
        } else {
            rightSibling = Optional.of(rightSiblingNum);
        }
