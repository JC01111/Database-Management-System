...
public class BPlusTree {
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        if(!temp.isPresent()){
            ...
            List<Long> rootChild = new ArrayList<>();
            rootKeys.add(temp.get().getFirst());
            rootChild.add(root.getPage().getPageNum());
            rootChild.add(temp.get().getSecond());

            this.updateRoot(new InnerNode(this.metadata, bufferManager, rootKeys, rootChild, lockContext));
        }
    }
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            return nextIter.hasNext() || (nextNode.getRightSibling().isPresent() && nextNode.getRightSibling().get().scanAll().hasNext());
        }
        ...
        public RecordId next() {
            ...
            if (nextIter.hasNext()) {
                ...
                nextNode = nextNode.getRightSibling().orElse(null);
                if (nextNode == null) {
                    throw new NoSuchElementException();
                }
