...
class InnerNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        ...
        int childIndex = numLessThanEqual(key, keys);
        BPlusNode child = getChild(childIndex);
        Optional<Pair<DataBox, Long>> temp = child.put(key, rid);
        int order = metadata.getOrder();

        //Chris: case1 coresponding to leafnode.java
        if(!temp.isPresent()){
            sync();
            return Optional.empty();
        }
        else{
            int newkeyLocation = numLessThanEqual(temp.get().getFirst(), keys);
            keys.add(newkeyLocation, temp.get().getFirst());
            children.add(newkeyLocation + 1, temp.get().getSecond());

            //Chris: case1 coresponding to leafnode.java
            if(keys.size() <= 2 * order){sync(); return Optional.empty();}
            //case2 coresponding to leafnode.java
            else{
                ...
                List<Long> leftChild = children.subList(0, order + 1);
                List<Long> rightChild = children.subList(order + 1, children.size());

                InnerNode right_node = new InnerNode(metadata, bufferManager, rightKeys,
                        rightChild, treeContext);
                this.keys = leftKeys;
                this.children = leftChild;

                sync();
                return Optional.of(new Pair<DataBox, Long>(split_key, right_node.getPage().getPageNum()));
            }
        }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while (data.hasNext()) {
            ...
            Optional<Pair<DataBox, Long>> temp = child.bulkLoad(data, fillFactor);
            ...
            if (!temp.isPresent()) {
                ...
                DataBox key = temp.get().getFirst();
                Long rid = temp.get().getSecond();
                keys.add(key);
                children.add(rid);
                sync();
                if (keys.size() == 2 * order) {
                    ...
                    InnerNode newNode = new InnerNode(metadata, bufferManager, rightKeys, rightChild, treeContext);
                    this.keys = leftKeys;
                    this.children = leftChild;
