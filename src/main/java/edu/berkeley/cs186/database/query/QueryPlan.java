...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        // Takes a table and return the optimal QueryOperator for scanning the table
        // SequentialScanOperator or IndexScanOperator
        int except = -1;
        int minIOCost = minOp.estimateIOCost();  // Set it to be temporary minIOCost
        for (int idx: getEligibleIndexColumns(table)) {
            SelectPredicate predicate = selectPredicates.get(idx);  // Get the predicate from idx
            QueryOperator indexScan = new IndexScanOperator(this.transaction, table, predicate.column, predicate.operator, predicate.value);
            int indexScanIOCost = indexScan.estimateIOCost();
            if (indexScanIOCost < minIOCost) {
                minIOCost = indexScanIOCost;
                except = idx;
                minOp = indexScan;
            }
        }
        minOp = addEligibleSelections(minOp, except); // Push down the selection predicates that involve solely the table
        return minOp;
    }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> table: prevMap.keySet()) {
            for (JoinPredicate joinPredicate: this.joinPredicates) {
                ...
                String leftSide = joinPredicate.leftTable;
                String rightSide = joinPredicate.rightTable;
                QueryOperator leftOp = null;
                QueryOperator rightOp = null;
                QueryOperator currOp;
                HashSet<String> newSet = new HashSet<>(table);
                // Case 1: Set contains leftTable but not right
                if (table.contains(leftSide) && !table.contains(rightSide)) {
                    for (Set<String> rightTable: pass1Map.keySet()) {   // Get rTable from here
                        if (rightTable.contains(rightSide)) {   // Check if this rightTable includes predicates we want
                            rightOp = pass1Map.get(rightTable);
                        }
                    }
                    leftOp = prevMap.get(table);
                    currOp = minCostJoinType(leftOp, rightOp, leftColumn, rightColumn);
                    newSet.add(rightSide);  // Add the rightSide to the newSet
                }
                // Case 2: Set contains rightTable but not left
                else if (!table.contains(leftSide) && table.contains(rightSide)) {
                    for (Set<String> leftTable: pass1Map.keySet()) {
                        if (leftTable.contains(leftSide)) {
                            leftOp = pass1Map.get(leftTable);
                        }
                    }
                    rightOp = prevMap.get(table);
                    currOp = minCostJoinType(rightOp, leftOp, rightColumn, leftColumn);  // Switch parameters to follow the left deep tree
                    newSet.add(leftSide);  // Add the leftSide to the newSet
                }
                // Case 3: skip this join predicate and continue the loop
                else {
                    continue;
                }
                // Maybe we want to compare two cases currOperator, use the min cost one
                int prevIOCost = Integer.MAX_VALUE;    // Previously joined tables IOCost
                if (result.containsKey(newSet)) {   // Compare with the previously joined tables
                    prevIOCost = result.get(newSet).estimateIOCost();
                    if (currOp.estimateIOCost() < prevIOCost) {
                        result.put(newSet, currOp);  // Adjust the result map with newSet and the best operator
                    }
                }
                result.put(newSet, currOp);
            }
        }
        return result;
    }
    ...
    public Iterator<Record> execute() {
        ...
        // Pass 1: find the lowest cost QueryOperator for each table
        Map<Set<String>, QueryOperator> pass1Map = new HashMap<>();
        for (String tableName: tableNames) {    // Loop over all tables to get their lowest cost operator
            QueryOperator lowestOp = minCostSingleAccess(tableName);
            Set<String> table = new HashSet<>();   // Cast the table name to Set<String>
            table.add(tableName);  // Update tables
            pass1Map.put(table, lowestOp); // Update the pass1Map
        }
        // Pass i
        Map<Set<String>, QueryOperator> prevMap = pass1Map;
        for (int i = 2; i <= tableNames.size(); i++) {
            prevMap = minCostJoins(prevMap, pass1Map);  // Find the lowest cost joins with each table from pass 1
        }
        // Final steps
